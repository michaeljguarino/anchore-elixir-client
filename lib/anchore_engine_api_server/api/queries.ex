...
defmodule AnchoreEngineAPIServer.Api.Queries do
  @moduledoc """
  ...
  - connection (AnchoreEngineAPIServer.Connection): Connection to server
  - name (String.t): Name of package to search for (e.g. sed)
  - opts (KeywordList): [optional] Optional parameters
    - :package_type (String.t): Type of package to filter on (e.g. dpkg)
    - :version (String.t): Version of named package to filter on (e.g. 4.4-1)
  ...
  {:ok, %AnchoreEngineAPIServer.Model.PaginatedImageList{}} on success
  {:error, info} on failure
  """
  @spec query_images_by_package(Tesla.Env.client, String.t, keyword()) :: {:ok, AnchoreEngineAPIServer.Model.PaginatedImageList.t} | {:error, Tesla.Env.t}
  def query_images_by_package(connection, name, opts \\ []) do
    optional_params = %{
      :package_type => :query,
      :version => :query,
      :page => :query,
    ...
    |> Enum.into([])
    |> (&Connection.request(connection, &1)).()
    |> decode(%AnchoreEngineAPIServer.Model.PaginatedImageList{})
  end
